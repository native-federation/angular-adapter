...
export async function createAngularEsbuildContext(options: NormalizedContextOptions): Promise<{
  ...
  const pluginOptions: CreateCompilerPluginParams[0] = {
  ...
  const stylesheetBundlerOptions: CreateCompilerPluginParams[1] & { inlineStyleLanguage: string } =
    {
      workspaceRoot,
      ...
      sass: builderOptions?.stylePreprocessorOptions?.sass as any,
