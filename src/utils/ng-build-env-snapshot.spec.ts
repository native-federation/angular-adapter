...
/** The values @angular/build freezes for a user who opted into chunk optimization. */
function defaultSnapshot(): Record<string, unknown> {
  return { useParallelTs: true, shouldOptimizeChunks: true, maxWorkers: 4 };
}
...
describe('replayNgBuildEnv', () => {
  ...
  it('re-applies both variables onto a stale snapshot', () => {
    ...
    expect(snapshot['shouldOptimizeChunks']).toBe(false);
    expect(snapshot['maxWorkers']).toBe(4);
    expect(messages).toHaveLength(1);
    expect(messages[0]?.level).toBe('info');
    expect(messages[0]?.message).toContain('useParallelTs=false, shouldOptimizeChunks=false');
  });
  ...
  it('only touches the variables it was asked about', () => {
    ...
    expect(snapshot['useParallelTs']).toBe(false);
    expect(snapshot['shouldOptimizeChunks']).toBe(true);
  });
  ...
  it('says nothing when the snapshot already matches', () => {
    const snapshot = { useParallelTs: false, shouldOptimizeChunks: false };
  ...
  it('patches every copy of @angular/build in the cache', () => {
    ...
    expect(first['shouldOptimizeChunks']).toBe(false);
    expect(second['shouldOptimizeChunks']).toBe(false);
  });
  ...
  it('warns instead of throwing when upstream renamed the export', () => {
    ...
    expect(messages[1]?.message).toContain(
      'shouldOptimizeChunks (expected a boolean, found undefined)'
    );
  });
  ...
  it('warns instead of throwing when upstream changed the type', () => {
    const messages = replayNgBuildEnv(
      ...
      cacheWith({ shouldOptimizeChunks: 'yes' }),
      ADAPTER_ENV
    );

    expect(messages[0]?.level).toBe('warn');
    expect(messages[0]?.message).toContain(
      'shouldOptimizeChunks (expected a boolean, found string)'
    );
  });

  it.each([
    ['an accessor', { get: () => true }],
    ['read-only', { value: true, writable: false }],
  ])('warns instead of throwing when the export became %s', (_label, descriptor) => {
    const snapshot = {};
    Object.defineProperty(snapshot, 'shouldOptimizeChunks', descriptor);
    ...
    expect(messages[0]?.level).toBe('warn');
    expect(messages[0]?.message).toContain('shouldOptimizeChunks (not writable)');
  });
